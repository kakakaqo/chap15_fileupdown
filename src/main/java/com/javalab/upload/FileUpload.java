 ...
 * 파일 업로드 라이브러리를 사용해야함.
 * 
 *  [필요한 라이브러리 - Apache Commons 파일 업로드 라이브러리] 
 *  1. commons-fileupload-1.3.3.jar 
 *  2. commons-io-2.6.jar 
 * 
 *  [enctype] 
 *  - enctype속성은 <form>태그의 데이터들을 전송할때 데이터들을 
 *    어떤 형식으로 변환할것인지에 대한 값을 지정 
 *  [multipart/form-data] 
...
public class FileUpload extends HttpServlet {
   ...
   private void doHandle(HttpServletRequest request, 
          ...
          * 이렇게 임시파일로 저장하는 이유는 대용량 파일을 업로드 했을때 그만한 크기의 
          * 데이터를 웹어플리케이션이 동작하는 JVM 메모리상에 모두 로드하는것은 부담이 
          * 되기 때문이다. 임시파일로 저장되었던 파일은 이후에 FileItem#write()
          * 메서드를 통해 실제 파일로 변경되거나 FileItem#delete()메서드를 통해 
          * 제거됨.           
          */
          ...
          * 객체로부터 multipart/form-data형식으로 넘어온 HTTP Body 부분을 다루기
          * 쉽게 변환(parse)해주는 역할을 수행합니다.  parseRequest()메서드를 
         ...
         try {
             ...
             *  즉 리턴값이 false인 경우에만 업로드된 파일인것으로 인지하여 처리.
             *  parseRequest()메서드 : FileItem을 ArrayList에 담아줌
             */            
            //             
            ...
            for (int i = 0; i < items.size(); i++) {
               ...
               if (fileItem.isFormField()) {   // text 같은 일반 입력 데이터(skip)
                  ...
                  if (fileItem.getSize() > 0) {
                     ...
                     // 파일이 업로될 곳의 풀경로를 갖는 File 객체 생성(경로 + 파일명)
                     ...
                     // 개발자가 하지 않고 write()메소드 차원에서 지원해주는 기능.
